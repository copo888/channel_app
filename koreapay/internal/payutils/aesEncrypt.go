...
//func AesEncrypt(src, key string) ([]byte, string) {
//	logx.Infof("加签参数：%s，加签KEY:%s", src, key)
...
func Md5V(str string, ctx context.Context) string {
	logx.WithContext(ctx).Infof("加簽前:%s", str)
	h := md5.New()
	h.Write([]byte(str))
	sing := strings.ToUpper(hex.EncodeToString(h.Sum(nil)))
	logx.WithContext(ctx).Infof("加簽後:%s", sing)
	return sing
}
