...
func VerifySign(reqSign string, data interface{}, screctKey string, ctx context.Context) bool {
	m := CovertToMapCallBack(data)
	source := JoinStringsInASCII(m, "&", false, true, screctKey)
	sign := GetSign_SHA256(source)
	fmt.Sprintf("-------" + source)
...
func SortAndSignFromObj(data interface{}, screctKey string, ctx context.Context) string {
	...
	logx.WithContext(ctx).Info("SHA256签名字串: ", newSign)
	return newSign
}
...
func CovertToMap(req interface{}) map[string]string {
	...
	for i := 0; i < val.Type().NumField(); i++ {
		...
		name := parts[0]
		if name != "sign" && name != "myIp" && name != "ip" && name != "channelNo" && name != "userName" && name != "appSecret" &&
			name != "payeeName" { // 過濾不需加簽參數
...
func CovertToMapCallBack(req interface{}) map[string]string {
	m := make(map[string]string)
