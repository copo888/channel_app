...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	channelQueryResp := struct {
		...
		Status     string  `json:"status"` //PENDING (待处理); PAID (已处理); REVERSED (已冲正); CANCELLED (已撤销)
	...
	if err3 := ChannelResp.DecodeJSON(&channelQueryResp); err3 != nil {
		...
		logx.WithContext(l.ctx).Errorf("代付查询渠道返回错误: %s: %s", channelQueryResp.Code, channelQueryResp.Text)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, channelQueryResp.Text)
	}
	//0:待處理 1:處理中 20:成功 30:失敗 31:凍結
	var orderStatus = "1"
	if channelQueryResp.Status == "PAID" {
		orderStatus = "20"
	} else if strings.Index("CANCELLED", channelQueryResp.Status) > -1 {
		orderStatus = "30"
	}
