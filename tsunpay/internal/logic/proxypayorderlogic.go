...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	data := struct {
		...
		BankCardHolderName string `json:"bank_card_holder_name"`
		...
		BankCardHolderName: req.ReceiptAccountName,
		BankCardNumber:     req.ReceiptAccountNumber,
		BankName:           channelBankMap.MapCode, //req.ReceiptCardBankName,
	...
	if ChnErr != nil {
		...
		if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
			...
			logx.WithContext(l.ctx).Errorf("写入交易日志错误:%s", err.Error())
		}
	...
	if strings.Index(channelResp.Message, "余额不足") > -1 {
		logx.WithContext(l.ctx).Errorf("代付渠提单道返回错误: %d: %s\n", channelResp.ErrorCode, channelResp.Message)
		//寫入交易日志
		if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
			MerchantNo:       req.MerchantId,
			MerchantOrderNo:  req.MerchantOrderNo,
			...
			TraceId:          l.traceID,
			ChannelErrorCode: fmt.Sprintf("%d", channelResp.ErrorCode),
		}); err != nil {
		...
		logx.WithContext(l.ctx).Errorf("代付渠道返回错误: %d: %s", channelResp.HttpStatusCode, channelResp.Message)
