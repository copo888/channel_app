...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	// 渠道狀態碼判斷
	if channelResp.ReturnCode != "IPS00000" {
		logx.WithContext(l.ctx).Errorf("支付渠道返回错误: %d: %+v", channelResp.ReturnCode, channelResp.Message)
		// 寫入交易日志
		if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
			MerchantNo:       req.MerchantId,
			ChannelCode:      channel.Code,
		...
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, fmt.Sprintf("%s", channelResp.Message))
	}
