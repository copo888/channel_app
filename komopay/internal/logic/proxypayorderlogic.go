...
import (
	...
	"github.com/copo888/channel_app/komopay/internal/payutils"
	"github.com/copo888/channel_app/komopay/internal/service"
	"github.com/gioco-play/gozzle"
	"go.opentelemetry.io/otel/trace"
	"strconv"
	"strings"

	"github.com/copo888/channel_app/komopay/internal/svc"
	"github.com/copo888/channel_app/komopay/internal/types"
...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	if channelResp.Success != true {
		...
		if channelResp.Errors != nil {
			...
			if strings.Index(message, "餘額不足") > -1 {
				logx.WithContext(l.ctx).Errorf("代付渠提单道返回错误: %d: %+v", channelResp.StatusCode, channelResp.Errors)
				return nil, errorx.New(responsex.INSUFFICIENT_IN_AMOUNT, fmt.Sprintf("%s", message))
			}

		}
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, fmt.Sprintf("%s", message))
	}
