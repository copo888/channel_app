...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.ClientNo, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo:      channel.MerId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.ClientNo, //輸入COPO訂單號
	...
	//PAYING	新建（请求订单成功，待支付）
	//PAID	已付（支付成功，且回调给商户）
	//FINISH	完成（商户收到PAID回调，且返回小写ok）
	//CANCEL	驳回（收款信息错误，驳回订单）
	//FAILED	失败（暂无该状态）
	//REVERT	冲正（已回调的PAID订单，因为1-2天内银行可能冲正，二次回调REVERT状态，并自动返还商户代付）
	var status = "0" //渠道回調狀態(0:處理中1:成功2:失敗)
	if req.Status == "FINISH" || req.Status == "PAID" {
		status = "1"
	} else if strings.Index("FAILED,CANCEL,REVERT", req.Status) > -1 {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.ClientNo,
		ChannelOrderNo:      req.Txid,
