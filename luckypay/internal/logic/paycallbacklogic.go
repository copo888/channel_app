...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.ClientNo, l.svcCtx.Config.ProjectName, req)
	...
	//CREATE 新建状态订单	PAID 已支付（订单入金成功，并回调给商户）	FINISH完成（商户收到PAID回调，并返回ok）	CANCEL	取消（当前不会有该状态）
	orderStatus := "1"
