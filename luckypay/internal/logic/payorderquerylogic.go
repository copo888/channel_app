package logic

import (
	"context"
	"fmt"
	"github.com/copo888/channel_app/common/errorx"
	"github.com/copo888/channel_app/common/model"
	"github.com/copo888/channel_app/common/responsex"
	"github.com/copo888/channel_app/common/typesX"
	"github.com/copo888/channel_app/luckypay/internal/payutils"
	"github.com/copo888/channel_app/luckypay/internal/svc"
	"github.com/copo888/channel_app/luckypay/internal/types"
	"github.com/gioco-play/gozzle"
	"github.com/zeromicro/go-zero/core/logx"
	"go.opentelemetry.io/otel/trace"
	"net/url"
)

type PayOrderQueryLogic struct {
	logx.Logger
	ctx     context.Context
	svcCtx  *svc.ServiceContext
	traceID string
}

func NewPayOrderQueryLogic(ctx context.Context, svcCtx *svc.ServiceContext) PayOrderQueryLogic {
	return PayOrderQueryLogic{
		Logger:  logx.WithContext(ctx),
		ctx:     ctx,
		svcCtx:  svcCtx,
		traceID: trace.SpanContextFromContext(ctx).TraceID().String(),
	}
}

func (l *PayOrderQueryLogic) PayOrderQuery(req *types.PayOrderQueryRequest) (resp *types.PayOrderQueryResponse, err error) {

	logx.WithContext(l.ctx).Infof("Enter PayOrderQuery. channelName: %s, PayOrderQueryRequest: %+v", l.svcCtx.Config.ProjectName, req)

	// 取得取道資訊
	var channel typesX.ChannelData
	channelModel := model.NewChannel(l.svcCtx.MyDB)
	if channel, err = channelModel.GetChannelByProjectName(l.svcCtx.Config.ProjectName); err != nil {
		return
	}
	// 組請求參數
	data := url.Values{}
	data.Set("clientNo", req.OrderNo)
	data.Set("clientCode", channel.MerId)

	// 加簽
	keys := []string{"clientCode", "clientNo"}
	sign := payutils.SortAndSignFromUrlValues(data, channel.MerKey, keys, l.ctx)
	data.Set("sign", sign)

	// 加簽 JSON
	//sign := payutils.SortAndSignFromObj(data, channel.MerKey)
	//data.sign = sign
	url := channel.PayQueryUrl + "?clientCode=" + channel.MerId + "&clientNo=" + req.OrderNo + "&sign=" + sign
	// 請求渠道
	logx.WithContext(l.ctx).Infof("支付查詢请求地址:%s,支付請求參數:%v", url, data)

	span := trace.SpanFromContext(l.ctx)
	res, chnErr := gozzle.Get(url).Timeout(20).Trace(span).Form(data)

	if chnErr != nil {
		return nil, errorx.New(responsex.SERVICE_RESPONSE_DATA_ERROR, err.Error())
	} else if res.Status() != 200 {
		logx.WithContext(l.ctx).Infof("Status: %d  Body: %s", res.Status(), string(res.Body()))
		return nil, errorx.New(responsex.INVALID_STATUS_CODE, fmt.Sprintf("Error HTTP Status: %d", res.Status()))
	}
	logx.WithContext(l.ctx).Infof("Status: %d  Body: %s", res.Status(), string(res.Body()))

	// 渠道回覆處理
	channelResp := struct {
		Success bool   `json:"success"`
		Code    int    `json:"code"`
		Message string `json:"message,optional"`
		Data    struct {
			OrderNo       string  `json:"orderNo,optional"`
			ShortOrderNo  string  `json:"shortOrderNo,optional"`
			ReceiveAddr   string  `json:"receiveAddr,optional"`
			ReceiveName   string  `json:"receiveName,optional"`
			ChainName     string  `json:"chainName,optional"`
			CoinUnit      string  `json:"coinUnit,optional"`
			RequestAmount float64 `json:"requestAmount,optional"`
			Status        string  `json:"status,optional"` //CREATE,PAID,FINISH,CANCEL,FAILED,CLOSETRAD
			PayUrl        string  `json:"payUrl,optional"`
			Hrefbackurl   string  `json:"hrefbackurl,optional"`
			Sign          string  `json:"sign,optional"`
		} `json:"data,optional"`
	}{}

	if err = res.DecodeJSON(&channelResp); err != nil {
		return nil, errorx.New(responsex.GENERAL_EXCEPTION, err.Error())
	} else if channelResp.Code != 200 || channelResp.Success != true {
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, channelResp.Message)
	}

	//orderAmount, errParse := strconv.ParseFloat(channelResp.Data.RequestAmount, 64)
	//if errParse != nil {
	//	return nil, errorx.New(responsex.GENERAL_EXCEPTION, errParse.Error())
	//}

	orderStatus := "0"
	if channelResp.Data.Status == "PAID" || channelResp.Data.Status == "FINISH" {
		orderStatus = "1"
	}

	resp = &types.PayOrderQueryResponse{
		OrderAmount: channelResp.Data.RequestAmount,
		OrderStatus: orderStatus, //订单状态: 状态 0处理中，1成功，2失败
	}

	return
}
