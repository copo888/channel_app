...
*/
func JoinStringsInASCII(data map[string]string, sep string, onlyValues, includeEmpty bool, key string, keys []string, exceptKeys ...string) string {
	var list []string
	var keyList []string
...
// VerifySign 验签
func VerifySign(reqSign string, data interface{}, screctKey string, keys []string, ctx context.Context) bool {
	m := CovertToMap(data)
	source := JoinStringsInASCII(m, "&", false, false, screctKey, keys)
	sign := GetSign(source)
	fmt.Sprintf("-------" + source)
...
func SortAndSignFromUrlValues(values url.Values, screctKey string, keys []string, ctx context.Context) string {
	m := CovertUrlValuesToMap(values)
	return SortAndSignFromMap(m, screctKey, keys, ctx)
}

func SortAndSignFromUrlValues_SHA256(values url.Values, screctKey string, keys []string) string {
	m := CovertUrlValuesToMap(values)
	return SortAndSignSHA256FromMap(m, screctKey, keys)
}

// SortAndSignFromObj 物件 排序后加签
func SortAndSignFromObj(data interface{}, screctKey string, keys []string, ctx context.Context) string {
	m := CovertToMap(data)
	newSource := JoinStringsInASCII(m, "&", false, false, screctKey, keys)
...
func SortAndSignFromMap(newData map[string]string, screctKey string, keys []string, ctx context.Context) string {
	newSource := JoinStringsInASCII(newData, "&", true, false, screctKey, keys)
	newSign := GetSign(newSource)
	logx.WithContext(ctx).Info("加签参数: ", newSource)
	logx.WithContext(ctx).Info("签名字串: ", newSign)
	return newSign
...
func SortAndSignSHA256FromMap(newData map[string]string, screctKey string, keys []string) string {
	newSource := JoinStringsInASCII(newData, "&", false, true, screctKey, keys)
