...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/copo888/channel_app/bitpayz2/internal/payutils"
	"github.com/copo888/channel_app/bitpayz2/internal/service"
...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	// 加簽
	sign, err := payutils.GetSign_HMAC_SHA256(channel.MerId, "7uGX2fbFqF", channel.MerKey, timestamp)
	if err != nil {
	...
	res, ChnErr := gozzle.Post(channel.PayUrl).Header("x-api-key", "ecda46d4-1261-490b-80d2-88a8b80a0748").Timeout(20).Trace(span).JSON(data)
	...
	if ChnErr != nil {
		...
		return nil, errorx.New(responsex.INVALID_STATUS_CODE, fmt.Sprintf("%s", string(res.Body())))
	}
	logx.WithContext(l.ctx).Infof("Status: %d  Body: %s", res.Status(), string(res.Body()))
