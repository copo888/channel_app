...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		...
		OrderNo:   req.Corderid, //輸入COPO訂單號
	...
	// 檢查驗簽
	source := req.MerchantId + req.Corderid + req.Money + channel.MerKey
	sign := payutils.GetSign(source)
	logx.WithContext(l.ctx).Info("verifySource: ", source)
	logx.WithContext(l.ctx).Info("verifySign: ", strings.ToUpper(sign))
	logx.WithContext(l.ctx).Info("reqSign: ", req.Sign)
	if req.Sign != strings.ToUpper(sign) {
