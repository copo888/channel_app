...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	//notifyUrl = "https://b2fa-211-75-36-190.jp.ngrok.io/api/pay-call-back"
	...
	data.Set("callbackurl", req.PageUrl)
	...
	// 加簽
	source := channel.MerId + req.OrderNo + req.ChannelPayType + notifyUrl + req.PageUrl + req.TransactionAmount + channel.MerKey
	sign := payutils.GetSign(source)
	logx.WithContext(l.ctx).Info("加签参数: ", source)
	logx.WithContext(l.ctx).Info("签名字串: ", sign)
	data.Set("realname", req.UserId)
	data.Set("userip", ip)
	data.Set("sign", sign)
