...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	if req.ReceiptCardBankCode != "002" {
		return nil, errorx.New(responsex.INVALID_PARAMETER, "不支援非maya出款")
	}
	...
	channelResp := struct {
		...
		Data    struct {
			...
			BankOwner     string  `json:"bank_owner, optional"`
	...
	if channelResp.Success != true {
		...
		if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
			...
			TraceId:          l.traceID,
			ChannelErrorCode: fmt.Sprintf("%s", channelResp.Message),
		}); err != nil {
			logx.WithContext(l.ctx).Errorf("写入交易日志错误:%s", err)
		}
