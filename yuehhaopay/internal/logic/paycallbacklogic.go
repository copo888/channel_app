...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", Result.Orderid, l.svcCtx.Config.ProjectName, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		MerchantNo: "",
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   Result.Orderid, //輸入COPO訂單號
	...
	if orderAmount, err = strconv.ParseFloat(Result.RealAmount, 64); err != nil {
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     Result.Orderid,
		ChannelOrderNo: Result.TransactionId, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
