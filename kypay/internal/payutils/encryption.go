...
func JoinStringsInASCII(data map[string]string, sep string, onlyValues, includeEmpty bool, key string, exceptKeys ...string) string {
	...
	return strings.Join(list, sep) + "&api_key=" + key
}
...
func SortAndSignSHA256FromObj(data interface{}, screctKey string, ctx context.Context) string {
	m := CovertToMap(data)
	newSign := SortAndSignSHA256FromMap(m, screctKey, ctx)
	return newSign
}
