...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequestX) (resp string, err error) {
	...
	if orderAmount, err = req.Amount.Float64(); err != nil { // 單位:分
	...
	if req.Status == "3" {
		status = "1"
	} else if strings.Index("4,5", req.Status.String()) > -1 {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.PayoutClId,
		ChannelOrderNo:      req.PayoutId,
