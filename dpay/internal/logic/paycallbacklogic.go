...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		...
		OrderNo:   req.CusOrderSn, //輸入COPO訂單號
	...
	//if orderAmount, err = strconv.ParseFloat(req.ReceiveAmount, 64); err != nil {
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.CusOrderSn,
		ChannelOrderNo: req.OrderSn, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
		OrderStatus:    orderStatus, // 若渠道只有成功会回调 固定 20:成功; 訂單狀態(1:处理中 20:成功 )
		OrderAmount:    req.ReceiveAmount,
