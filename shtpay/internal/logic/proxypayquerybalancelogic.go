...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	type Result struct {
		Availableamount float64 `mapstructure:"availableamount"`
	}

	if err3 := ChannelResp.DecodeJSON(&balanceQueryResp); err3 != nil {
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, err3.Error())
	} else if balanceQueryResp.Code != 0 {
		msg := fmt.Sprint(balanceQueryResp.Results)
		logx.WithContext(l.ctx).Errorf("代付余额查询渠道返回错误: %s: %s", balanceQueryResp.Code, msg)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, msg)
	}

	results := Result{}
	if err = mapstructure.Decode(balanceQueryResp.Results, &results); err != nil {
		return nil, errorx.New(responsex.SYSTEM_ERROR, "格式转换错误")
	}

	balance := fmt.Sprintf("%f", results.Availableamount)
