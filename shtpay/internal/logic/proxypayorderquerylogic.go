...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	if err3 := ChannelResp.DecodeJSON(&channelQueryResp); err3 != nil {
		...
		msg := fmt.Sprint(channelQueryResp.Results)
		logx.WithContext(l.ctx).Errorf("代付查询渠道返回错误: %s: %s", channelQueryResp.Code, msg)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, msg)
	}

	results := Result{}
	if err = mapstructure.Decode(channelQueryResp.Results, &results); err != nil {
