...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v",
		req.Outtradeno, l.svcCtx.Config.ProjectName, req)
	...
	if orderAmount, err = strconv.ParseFloat(req.Ramount, 64); err != nil {
