...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v",
		l.svcCtx.Config.ProjectName, req.Outtransferno, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo:      channel.MerId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.Outtransferno, //輸入COPO訂單號
	...
	if orderAmount, err = strconv.ParseFloat(req.Transferamount, 64); err != nil {
		return "fail", errorx.New(responsex.INVALID_SIGN)
	}
	...
	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.Outtransferno,
		ChannelOrderNo:      "",
