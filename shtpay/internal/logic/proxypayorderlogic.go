...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	data.Set("subbankname", req.ReceiptCardBranch)
	...
	data.Set("outtransferno", req.OrderNo)
	...
	if strings.Index(channelResp.Results, "not enough amount") > -1 {
		logx.WithContext(l.ctx).Errorf("代付渠提单道返回错误: %s: %s", channelResp.Code, channelResp.Results)
		return nil, errorx.New(responsex.INSUFFICIENT_IN_AMOUNT, channelResp.Results)
	} else if channelResp.Code != 0 {
		logx.WithContext(l.ctx).Errorf("代付渠道返回错误: %s: %s", channelResp.Code, channelResp.Results)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, channelResp.Results)
	}
