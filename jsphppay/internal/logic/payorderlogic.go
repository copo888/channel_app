...
import (
	...
	"github.com/copo888/channel_app/jsphppay/internal/service"
	"github.com/copo888/channel_app/jsphppay/internal/svc"
	"github.com/copo888/channel_app/jsphppay/internal/types"
...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	if !channelResp.Success {
		logx.WithContext(l.ctx).Errorf("支付渠道返回错误: %d: %+v", channelResp.Success, channelResp.Message)
		// 寫入交易日志
		if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
			MerchantNo:       req.MerchantId,
			ChannelCode:      channel.Code,
			...
			TraceId:          l.traceID,
			ChannelErrorCode: fmt.Sprintf("%d", channelResp.Success),
		}); err != nil {
			logx.WithContext(l.ctx).Errorf("写入交易日志错误:%s", err)
		}
