...
func JoinStringsInASCII(data map[string]string, sep string, onlyValues, includeEmpty bool, key string, exceptKeys ...string) string {
	...
	return strings.Join(list, sep) + "&secret_key=" + key
}

// VerifySign 验签 reqType[依据不同请求类型更改] from / json
func VerifySign(reqSign string, data interface{}, screctKey string, reqType string) bool {
	m := CovertToMap(data, reqType)
...
func CovertToMap(req interface{}, reqType string) map[string]string {
	m := make(map[string]string)
