...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		...
		OrderNo:     req.OrderNumber, //輸入COPO訂單號
	...
	// 檢查驗簽
	//if isSameSign := payutils.VerifySign(req.Sign, *req, channel.MerKey, "json"); !isSameSign {
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.OrderNumber,
		ChannelOrderNo: "CHN_" + req.OrderNumber, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
