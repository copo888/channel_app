...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	sign := utils.GetSign(fmt.Sprintf("productid=%s&key=%s", channel.MerId, channel.MerKey))
	data.Sign = sign
	// 請求渠道
	logx.WithContext(l.ctx).Infof("代付余额查询请求地址:%s", channel.ProxyPayQueryBalanceUrl)
	span := trace.SpanFromContext(l.ctx)
	ChannelResp, ChnErr := gozzle.Post(channel.ProxyPayQueryBalanceUrl).Header("Authorization", "Bearer "+channel.MerKey).
