...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	ct := payutils.DecodingRSA([]byte(req.Context), channel.MerKey)

	json.Unmarshal([]byte(ct),reqCon)

	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, reqCon.OrderNo, reqCon)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo:      channel.MerId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   reqCon.OrderNo, //輸入COPO訂單號
	...
	// 檢查驗簽
	sign := payutils.GetSign(channel.MerKey)
	if req.Sign != sign {
	...
	if reqCon.OrderState == "1" {
		status = "1"
	} else if strings.Index("2,4", reqCon.OrderState) > -1 {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     reqCon.OrderNo,
		ChannelOrderNo:      "",
