...
*/
func JoinStringsInASCII(data map[string]string, keys []string, sep string, onlyValues, includeEmpty bool, key string, exceptKeys ...string) string {
	var list []string
	var keyList []string
...
//	newSign := GetSign_HMAC_SHA1(newSource)
...
func GetSign_HMAC_SHA1(source string, key string) string {
	key_for_sign := []byte(key)
	mac := hmac.New(sha1.New, key_for_sign)
	mac.Write([]byte(source))
	return hex.EncodeToString((mac.Sum(nil)))
}

func GetSign_SHA1_Hash(source string) string {
