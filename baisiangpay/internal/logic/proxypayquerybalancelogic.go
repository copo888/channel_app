...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	ChannelResp, ChnErr := gozzle.Post(channel.ProxyPayQueryBalanceUrl).Timeout(10).Trace(span).JSON(data)
	...
	balanceQueryResp := struct {
		...
		Data    struct {
			PayBalance string `json:"pay_balance"`
		} `json:"data"`
	}{}

	if err3 := ChannelResp.DecodeJSON(&balanceQueryResp); err3 != nil {
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, err3.Error())
	} else if balanceQueryResp.Message != "success" {
		logx.Errorf("代付余额查询渠道返回错误: %s: %s", balanceQueryResp.Code, balanceQueryResp.Message)
	...
	resp = &types.ProxyPayQueryInternalBalanceResponse{
		...
		ProxyPayBalance:    balanceQueryResp.Data.PayBalance,
