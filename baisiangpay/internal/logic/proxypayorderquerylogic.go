...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	} else if channelQueryResp.Message != "success" {
		logx.Errorf("代付查询渠道返回错误: %s: %s", channelQueryResp.Code, channelQueryResp.Message)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, channelQueryResp.Message)
	}
	//0:待處理 1:處理中 20:成功 30:失敗 31:凍結
	var orderStatus = "1"                           // 30000 :表示待付款,
	if channelQueryResp.Data.PayStatus == "30001" { // 30001 :表示已完成
		orderStatus = "20"
	} else if channelQueryResp.Data.PayStatus == "30002" { // 30002 .表示已驳回
		orderStatus = "30"
	}
	...
	return &types.ProxyPayOrderQueryResponse{
		Status:           1,
		ChannelOrderNo:   channelQueryResp.Data.PayTransactionId,
