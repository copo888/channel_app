...
func (l *PayCallBackLogic) PayCallBack(req types.PayCallBackRequest) (string, error) {

	// 取得取道資訊
	channelModel := model2.NewChannel(l.svcCtx.MyDB)
	channel, err := channelModel.GetChannelByProjectName(l.svcCtx.Config.ProjectName)
	if err != nil {
		return "err", errorx.New(responsex.INVALID_PARAMETER, err.Error())
	}

	// 檢查白名單
	if isWhite := utils.IPChecker(req.MyIp, channel.WhiteList); !isWhite {
		return "err", errorx.New(responsex.IP_DENIED, "IP: "+req.MyIp)
	}
	...
	payCallBackBO := bo.PayCallBackBO{
		...
		ChannelOrderNo: "CHN_" + req.OutTradeNo, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
	...
	url := "http://127.0.0.1:8080/dior/merchant-api/pay-call-back" // TODO: URL要抽出来
