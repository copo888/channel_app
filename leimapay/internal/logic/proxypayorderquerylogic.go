...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	if err3 := ChannelResp.DecodeJSON(&channelQueryResp); err3 != nil {
		...
		logx.WithContext(l.ctx).Errorf("代付查询渠道返回错误: %s: %s", channelQueryResp.result, channelQueryResp.ErrorMsg.ErrorMsg)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, channelQueryResp.ErrorMsg.ErrorMsg)
	}
	//0:待處理 1:處理中 20:成功 30:失敗 31:凍結
	var orderStatus = "1"
	if channelQueryResp.Data.Status == "Success" {
		orderStatus = "20"
	} else if strings.Index("Failed, Unpaid", channelQueryResp.Data.Status) > -1 {
		orderStatus = "30"
	}
