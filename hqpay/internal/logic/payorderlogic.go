...
import (
	...
	"github.com/copo888/channel_app/hqpay/internal/service"
...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	notifyUrl := "http://f6a3-211-75-36-190.ngrok-free.app/api/pay-call-back"
	...
	data.Set("paymentType", req.ChannelPayType) // 1: 微信扫码	2: 支付宝	6: 银行卡转银行卡
	data.Set("username", req.UserId)
	data.Set("depositRealname", req.UserId)
	data.Set("callback", notifyUrl)
	...
	if req.JumpType == "json" {
		data.Set("useCounter", "false")
	} else {
		data.Set("useCounter", "true")
	}
	...
	channelResp := struct {
		...
		Whether string `json:"whether"` // YES 、 NO
		Code    int64  `json:"code"`
		Msg     string `json:"message, optional"`
		PayInfo struct {
			Price  float64 `json:"price, optional"`
			Url    string  `json:"redirect, optional"`
			Status int     `json:"statusSt, optional"`
			//收款人訊息
			Name      string `json:"recRealname, optional"`
			Card      string `json:"recAccount, optional"`
			Bank      string `json:"recBank, optional"`
	...
	if strings.EqualFold(req.JumpType, "json") {
		//amount, err2 := strconv.ParseFloat(channelResp.PayInfo.Price, 64)
		//if err2 != nil {
		...
		receiverInfoJson, err3 := json.Marshal(types.ReceiverInfoVO{
			...
			Amount:     channelResp.PayInfo.Price,
			Link:       "",
			Remark:     "",
		})
		if err3 != nil {
	...
	resp = &types.PayOrderResponse{
		PayPageType:    "url",
		PayPageInfo:    channelResp.PayInfo.Url,
		ChannelOrderNo: "",
	}
