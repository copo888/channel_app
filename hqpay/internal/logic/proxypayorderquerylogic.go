...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	channelQueryResp := struct {
		...
		data    struct {
			OutTradeNo string  `json:"reference"`
			Amount     float64 `json:"amount"`
			Status     string  `json:"status"` //PENDING，待处理。LOCKED，在处理。AUDITED，已支付。REFUSED，已拒绝。
		} `json:"data,optional"`
	}{}
	...
	var orderStatus = "1"
	if channelQueryResp.data.Status == "AUDITED" {
		orderStatus = "20"
	} else if channelQueryResp.data.Status == "REFUSED" {
		orderStatus = "30"
	}
	...
	return &types.ProxyPayOrderQueryResponse{
		Status: 1,
		//CallBackStatus: ""
		ChannelOrderNo:   channelQueryResp.data.OutTradeNo,
