...
func (l *ProxyPayOrderQueryLogic) ProxyPayOrderQuery(req *types.ProxyPayOrderQueryRequest) (resp *types.ProxyPayOrderQueryResponse, err error) {
	...
	channelQueryResp := struct {
		...
		AgentpayOrderId string `json:"agentpayOrderId, optional"`
		Status          string `json:"status, optional"`
	}{}

	if err3 := ChannelResp.DecodeJSON(&channelQueryResp); err3 != nil {
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, err3.Error())
	} else if channelQueryResp.RetCode != "SUCCESS" != true {
	...
	var orderStatus = "1"
	if channelQueryResp.Status == "4" {
		orderStatus = "20"
	} else if strings.Index("3,5", channelQueryResp.Status) > -1 {
		orderStatus = "30"
	}
