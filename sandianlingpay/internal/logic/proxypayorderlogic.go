...
func (l *ProxyPayOrderLogic) ProxyPayOrder(req *types.ProxyPayOrderRequest) (*types.ProxyPayOrderResponse, error) {
	...
	if err2 != nil { //BankName空: COPO沒有對應銀行(要加bk_banks)，MapCode為空: 渠道沒有對應銀行代碼(要加ch_channel_banks)
		logx.Errorf("銀行代碼抓取資料錯誤:%s", err2.Error())
	...
	data.Set("notifyurl", "https://addc-211-75-36-190.jp.ngrok.io/api/proxy-pay-call-back")
	data.Set("bankcode", channelBankMap.MapCode)
	data.Set("accprovince", req.ReceiptCardProvince)
	...
	} else if channelResp.ErrorCode != 0 {
		logx.Errorf("代付渠道返回错误: %s: %s", channelResp.ErrorCode, channelResp.ErrorMsg)
