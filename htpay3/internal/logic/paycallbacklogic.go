...
import (
	...
	"github.com/copo888/channel_app/htpay3/internal/svc"
	"github.com/copo888/channel_app/htpay3/internal/types"
...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.Mhtorderno, l.svcCtx.Config.ProjectName, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		MerchantNo: "",
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.Mhtorderno, //輸入COPO訂單號
	...
	//if orderAmount, err = strconv.ParseFloat(req.Paidamount, 64); err != nil {
	//	return "fail", errorx.New(responsex.INVALID_AMOUNT)
	//}

	orderStatus := "1"
	if req.Status == "1" { // 0：未付款	1：已付款	2：超时	3：付款失败
		orderStatus = "20"
	}
