...
import (
	...
	"github.com/copo888/channel_app/jinpinpay2/internal/payutils"
	"github.com/gioco-play/gozzle"
	"go.opentelemetry.io/otel/trace"
	"strconv"
	"time"

	"github.com/copo888/channel_app/jinpinpay2/internal/svc"
	"github.com/copo888/channel_app/jinpinpay2/internal/types"
...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.OrderNo, l.svcCtx.Config.ProjectName, req)
	...
	// 檢查驗簽
	source := req.Currency + req.Status + channel.MerId + req.OrderNo + req.Amount + channel.MerKey
	...
	payCallBackBO := bo.PayCallBackBO{
		...
		ChannelOrderNo: req.OutOrder, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
