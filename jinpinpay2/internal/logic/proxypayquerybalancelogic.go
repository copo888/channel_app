...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	// 加簽
	signString := channel.MerId + timestamp + "balance" + channel.MerKey
	...
	} else if balanceQueryResp.Status != 1 {
		logx.WithContext(l.ctx).Errorf("代付余额查询渠道返回错误: %s: %s", balanceQueryResp.Status, balanceQueryResp.Msg)
	...
	resp = &types.ProxyPayQueryInternalBalanceResponse{
		...
		ProxyPayBalance:    fmt.Sprintf("%.2f", balanceQueryResp.Money),
