...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	data.Add("productId", req.ChannelPayType)
	data.Add("mchOrderNo", req.OrderNo)
	data.Add("currency", strings.ToLower(req.Currency))
	data.Add("amount", fmt.Sprintf("%d", amountInt)) // 假设 amountInt 已经是一个整数
	//data.Add("clientIp", ip)
	data.Add("notifyUrl", notifyUrl)
	data.Add("subject", "COPO")
	data.Add("body", "COPO")
	data.Add("version", "1.0")
	data.Add("reqTime", timestamp)
	data.Add("param2", req.UserId)
	...
	channelResp := struct {
		...
		PayJumpUrl string `json:"payJumpUrl, optional"`
	}{}
