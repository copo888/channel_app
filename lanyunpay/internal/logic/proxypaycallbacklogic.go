...
import (
	...
	"github.com/copo888/channel_app/lanyunpay/internal/payutils"
	"github.com/gioco-play/gozzle"
	"go.opentelemetry.io/otel/trace"
	"strconv"
	"time"

	"github.com/copo888/channel_app/lanyunpay/internal/svc"
	"github.com/copo888/channel_app/lanyunpay/internal/types"
...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	source := req.FxStatus + req.FxId + req.FxDdh + req.FxFee + channel.MerKey
	verifySign := payutils.GetSign(source)
	logx.Info("verifySource: ", source)
	logx.Info("verifySign: ", verifySign)
	logx.Info("reqSign: ", req.FxSign)

	// 檢查驗簽
	if req.FxSign != verifySign {
		return "fail", errorx.New(responsex.INVALID_SIGN)
	}

	var orderAmount float64
	if orderAmount, err = strconv.ParseFloat(req.FxFee, 64); err != nil {
		return "fail", errorx.New(responsex.INVALID_SIGN)
	}
	var status = "0" //渠道回調狀態(0:處理中1:成功2:失敗)
	if req.FxStatus == "1" {
		status = "1"
	} else if req.FxStatus == "0" {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.FxDdh,
		ChannelOrderNo:      "",
