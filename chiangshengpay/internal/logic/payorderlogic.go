...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	//newSource := payutils.JoinStringsInASCII(m, "&", false, false, channel.MerKey)
	...
	// 請求渠道
	logx.WithContext(l.ctx).Infof("支付下单请求地址:%s,支付請求參數:%+v", channel.PayUrl, params)
	span := trace.SpanFromContext(l.ctx)
	res, ChnErr := gozzle.Post(channel.PayUrl).Timeout(20).Trace(span).JSON(params)
	...
	resp = &types.PayOrderResponse{
		PayPageType:    "url",
		PayPageInfo:    channelResp2.Result.ReturnUrl,
		ChannelOrderNo: "",
	}
