...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.FundOutPaymentReference, req)
	...
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		...
		OrderNo:     req.FundOutPaymentReference, //輸入COPO訂單號
	...
	if req.FundOutStatus == "PAID" {
		status = "1"
	} else if req.FundOutStatus == "REJECTED" {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.FundOutPaymentReference,
		ChannelOrderNo:      req.TransactionRef1,
		ChannelResultAt:     time.Now().Format("20060102150405"),
		ChannelResultStatus: status,
		ChannelResultNote:   req.FundOutDescription,
		Amount:              req.Amount,
		ChannelCharge:       0,
		UpdatedBy:           "",
	}
	...
	jsonStr, err := json.Marshal(resp2)
	if err != nil {
		logx.WithContext(l.ctx).Errorf("JSON 编码失败: %v", err)
	}
