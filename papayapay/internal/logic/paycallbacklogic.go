...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.QrCodeTransactionId, l.svcCtx.Config.ProjectName, req)
	...
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		...
		OrderNo:     req.QrCodeTransactionId, //輸入COPO訂單號
	...
	orderStatus := "1"
	if req.Status == "02_Paid" { //01_Pending 02_Paid 04_Refunded
		orderStatus = "20"
	}

	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.QrCodeTransactionId,
		ChannelOrderNo: req.TransactionRef1, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
	...
	resp2 := struct {
		...
		Status:              200,
		QrCodeTransactionId: req.QrCodeTransactionId,
		Message:             "success",
	}
