...
import (
	...
	"github.com/copo888/channel_app/leileipay/internal/service"
...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	// 請求渠道
	logx.WithContext(l.ctx).Infof("支付下单请求地址:%s,支付請求:%s,提交參數:%#v", channel.PayUrl, string(dataBytes), reqObj)
	...
	channelResp := struct {
		Code    int     `json:"code"`
		Msg     string  `json:"msg"`
		Toast   string  `json:"toast, optional"`
	...
	if channelResp.Code != 0 {
		...
		logx.WithContext(l.ctx).Errorf("支付渠道返回错误: %d: %s", channelResp.Code, channelResp.Msg)
		// 寫入交易日志
		if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
			MerchantNo:       req.MerchantId,
			ChannelCode:      channel.Code,
