...
func VerfiyCallBackSign(reqSign string, data interface{}, screctKey string, ctx context.Context) bool {
	m := CovertToMapForCallBack(data)
	source := JoinStringsInASCII(m, "&", false, false, screctKey)
	sign := GetSign(source)
	fmt.Sprintf("-------" + source)
...
func CovertToMap(req interface{}) map[string]string {
	...
	for i := 0; i < val.Type().NumField(); i++ {
		...
		name := parts[0]
		if name != "sign" && name != "myIp" && name != "ip" && name != "currencyCode" && name != "mp" && name != "payerName" { // 過濾不需加簽參數
...
func CovertToMapForCallBack(req interface{}) map[string]string {
	m := make(map[string]string)
