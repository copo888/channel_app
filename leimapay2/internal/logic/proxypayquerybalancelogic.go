...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	if err3 := ChannelResp.DecodeJSON(&balanceQueryResp); err3 != nil {
		...
		logx.WithContext(l.ctx).Errorf("代付余额查询渠道返回错误: %s: %s", balanceQueryResp.Result, balanceQueryResp.ErrorMsg.ErrorMsg)
		return nil, errorx.New(responsex.CHANNEL_REPLY_ERROR, balanceQueryResp.ErrorMsg.ErrorMsg)
	}
