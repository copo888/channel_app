...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.Merchanttradeno, req)
	...
	if isWhite := utils.IPChecker(req.Ip, channel.WhiteList); !isWhite {
		...
		return "ERROR", errorx.New(responsex.IP_DENIED, "IP: "+req.Ip)
	}
	// 檢查驗簽
	...
	if req.Tradestatus == "WITHDRAWAL_SUCCESS" {
		status = "1"
	} else if strings.Index("WITHDRAWAL_FAILURE", req.Tradestatus) > -1 {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.Merchanttradeno,
		ChannelOrderNo:      "",
