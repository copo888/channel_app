...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.Tradeno, req)
	...
	if req.Status == 4 { //交易状态: 1:提交成功, 2:处理中, 3: 取消订单, 4:交易成功, 5:交易异常
		status = "1"
	} else if req.Status == 3 || req.Status == 5 {
		status = "2"
	}
