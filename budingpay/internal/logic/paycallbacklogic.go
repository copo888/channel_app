...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.OrderId, req.MerchId, l.svcCtx.Config.ProjectName, req)
	...
	orderStatus := "1"
	if req.Status == "1" { //1成功，2失败，3风控，4未支付/支付中，5拒付，6退款/退货
		orderStatus = "20"
	}
