...
import (
	...
	"github.com/copo888/channel_app/budingpay/internal/service"
...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayOrder. orderNo:%s, channelName: %s, PayOrderRequest: %+v", req.OrderNo, l.svcCtx.Config.ProjectName, req)
	...
	//notifyUrl := "http://44b5-211-75-36-190.ngrok.io/api/pay-call-back"
	...
	// 組請求參數
	data := url.Values{}
	data.Set("method", "order")
	data.Set("app_id", channel.MerId)
	data.Set("order_sn", req.OrderNo)
	data.Set("order_amount", req.TransactionAmount)
	data.Set("user_name", req.UserId)
	...
	resp = &types.PayOrderResponse{
		PayPageType:    "url",
		PayPageInfo:    channelResp2.PayInfo.Url,
		ChannelOrderNo: "",
	}
