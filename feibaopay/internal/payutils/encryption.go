...
func GetSignAES256CBC(plaintext, key, iv string, blockSize int, ctx context.Context) string {
	bKey := []byte(key)
	bIV := []byte(iv)
	...
	logx.WithContext(ctx).Infof("加签参数: %+v, hash_key: %s, hash_iv: %s", plaintext, key, iv)
	logx.WithContext(ctx).Infof("签名字串: %s", res)
	return res
}
...
func AES256Decode(cipherText, key, iv string) (resp string, err error) {
	bKey := []byte(key)
	bIV := []byte(iv)
