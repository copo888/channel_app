...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.PlatformOrderNumber, l.svcCtx.Config.ProjectName, req)
	...
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		...
		OrderNo:     req.PlatformOrderNumber, //輸入COPO訂單號
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.PlatformOrderNumber,
		ChannelOrderNo: req.OrderNumber, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
		OrderStatus:    orderStatus,     // 若渠道只有成功会回调 固定 20:成功; 訂單狀態(1:处理中 20:成功 )
		OrderAmount:    req.AmountReceive,
