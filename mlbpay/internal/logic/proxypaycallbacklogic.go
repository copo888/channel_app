...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		...
		ChannelResultNote:   "",
		Amount:              utils.GetDecimal_Float(orderAmount),
		ChannelCharge:       0,
		UpdatedBy:           "",
	}
	...
	res, errx := gozzle.Post(url).Timeout(20).Trace(span).Header("authenticationProxykey", proxyKey).JSON(proxyPayCallBackBO)
	logx.Info("回调后资讯: ", res)
	if errx != nil {
