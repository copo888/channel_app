...
func (l *PayOrderLogic) PayOrder(req *types.PayOrderRequest) (resp *types.PayOrderResponse, err error) {
	...
	/** UserId 必填時使用 **/
	if strings.EqualFold(req.PayType, "UPI") && len(req.UserId) == 0 {
	...
	//notifyUrl := "https://dca2-211-75-36-190.ngrok-free.app/api/pay-call-back"
	...
	data := struct {
		...
		Payer     string `json:"depositor_name"`
	...
	channelResp := struct {
		...
		Upi        string `json:"upi,optional"`
		Qrcode     string `json:"qrcode,optional"`
		PaytmLink  string `json:"paytm_link,optional"`
