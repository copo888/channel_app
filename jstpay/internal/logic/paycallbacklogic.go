...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	// 檢查驗簽
	//if isSameSign := payutils.VerifySign(req.SignCode, *req, channel.MerKey); !isSameSign {
	//	return "fail", errorx.New(responsex.INVALID_SIGN)
	//}

	var orderAmount float64
	if orderAmount, err = strconv.ParseFloat(req.PayValue, 64); err != nil {
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.BillSerialFromOutside,
		ChannelOrderNo: req.BillSerial, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
	...
	result.Success = true
	result.Result.BillSerial = req.BillSerial
