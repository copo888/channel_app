...
import (
	...
	_ "github.com/copo888/channel_app/onepay/internal/payutils"
...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.MerchantOrderId, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo:      channel.MerId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.MerchantOrderId, //輸入COPO訂單號
	...
	if req.Status == 0 { //0：成功, 1：失败, 2：处理中
		status = "1"
	...
	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		...
		ChannelResultNote:   fmt.Sprintf("%d", req.Status),
		Amount:              req.Amount,
		ChannelCharge:       0,
		UpdatedBy:           "",
	}
