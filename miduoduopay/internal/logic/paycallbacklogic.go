...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.TranCode, l.svcCtx.Config.ProjectName, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo: req.MerchId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.TranCode, //輸入COPO訂單號
	...
	// 檢查驗簽
	if isSameSign := payutils.VerifySign(req.SignedMsg, *req, channel.MerKey, l.ctx); !isSameSign {
		return "fail", errorx.New(responsex.INVALID_SIGN)
	}

	var orderAmount float64
	if orderAmount, err = strconv.ParseFloat(req.OrigAmt, 64); err != nil {
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.TranCode,
		ChannelOrderNo: req.SysCode, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
