...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.Data.OrderNumber, l.svcCtx.Config.ProjectName, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo: req.MerchId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.Data.OrderNumber, //輸入COPO訂單號
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.Data.OrderNumber,
		ChannelOrderNo: req.Data.SystemOrderNumber, // 渠道訂單號 (若无则填入->"CHN_" + orderNo)
