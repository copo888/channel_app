...
import (
	...
	"github.com/copo888/channel_app/rcpay_ue/internal/svc"
	"github.com/copo888/channel_app/rcpay_ue/internal/types"
...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.Data.OrderNumber, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo:      channel.MerId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.Data.SystemOrderNumber, //輸入COPO訂單號
	...
	// 檢查驗簽
	//if isSameSign := payutils.VerifySign(req.Data.Sign, req.Data, channel.MerKey, l.ctx); !isSameSign {
	...
	if req.Data.Status == 4 || req.Data.Status == 5 {
		status = "1"
	} else if req.Data.Status == 6 || req.Data.Status == 7 || req.Data.Status == 8 {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.Data.OrderNumber,
		ChannelOrderNo:      "",
