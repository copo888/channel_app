...
func (l *ProxyPayCallBackLogic) ProxyPayCallBack(req *types.ProxyPayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter ProxyPayCallBack. channelName: %s, orderNo: %s, ProxyPayCallBackRequest: %+v", l.svcCtx.Config.ProjectName, req.MerchantOrderNo, req)

	//寫入交易日志
	if err := utils.CreateTransactionLog(l.svcCtx.MyDB, &typesX.TransactionLogData{
		//MerchantNo:      channel.MerId,
		//MerchantOrderNo: req.OrderNo,
		OrderNo:   req.MerchantOrderNo, //輸入COPO訂單號
	...
	amountF := utils.FloatDivF(orderAmount, 100)

	var status = "0" //渠道回調狀態(0:處理中1:成功2:失敗)
	if req.OrderStatus == "COMPLETED" {
		status = "1"
	} else if strings.Index("ERROR,TIMEOUT,CANCELED", req.OrderStatus) > -1 {
		status = "2"
	}

	proxyPayCallBackBO := &bo.ProxyPayCallBackBO{
		ProxyPayOrderNo:     req.MerchantOrderNo,
		ChannelOrderNo:      req.PlatformOrderNo,
