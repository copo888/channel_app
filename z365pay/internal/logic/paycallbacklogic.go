...
func (l *PayCallBackLogic) PayCallBack(req *types.PayCallBackRequest) (resp string, err error) {
	...
	logx.WithContext(l.ctx).Infof("Enter PayCallBack. orderNo:%s, channelName: %s, PayCallBackRequest: %+v", req.MerchantOrderNo, l.svcCtx.Config.ProjectName, req)
	...
	orderAmount = float64(req.ActualAmount)
	...
	payCallBackBO := bo.PayCallBackBO{
		PayOrderNo:     req.MerchantOrderNo,
