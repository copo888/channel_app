...
func (l *ProxyPayQueryBalanceLogic) ProxyPayQueryBalance() (resp *types.ProxyPayQueryInternalBalanceResponse, err error) {
	...
	} else if balanceQueryResp.Code != "SUCCESS" {
	...
	availableAmount := float64(balanceQueryResp.Available)
	amountF := utils.FloatDivF(availableAmount, 100)
	amountStr := strconv.FormatFloat(amountF, 'f', 2, 64)
